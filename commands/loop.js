...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (!queue || !queue.playing) return interaction.reply({ content: '⚠️ Nenhuma musica tocando!!', ephemeral: true }).catch(e => { })
      ...
      const embed = new EmbedBuilder()
        .setColor('#fc4e03')
        .setAuthor({
        name: 'Faça um loop em suas musicas',
      ...
      interaction?.reply({ embeds: [embed], components: [button], fetchReply: true }).then(async Message => {
        ...
        col.on('collect', async (button) => {
          ...
          if (!queue1 || !queue1.playing) {
            await interaction?.editReply({ content: '⚠️ Nenhuma música tocando!!', ephemeral: true }).catch(e => { })
            await button?.deferUpdate().catch(e => {})
          }
          switch (button.customId) {
            case 'queue':
              const success = queue.setRepeatMode(2);
              interaction?.editReply({ content: `✅ Fila em looping!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'nowplaying':
            ...
            case 'close':
              if (queue.repeatMode === 0) {
                await button?.deferUpdate().catch(e => {})
                return interaction?.editReply({ content: '⚠️ Looping já desligado!!', ephemeral: true }).catch(e => { })
              }
