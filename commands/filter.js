...
module.exports = {
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (!queue || !queue?.playing) return interaction?.reply({ content: '⚠️ Nenhum musica tocando!!', ephemeral: true }).catch(e => { })
      ...
      .setDescription('** Explore o melhor, escolha seu efeitoabaixo!**')
     ...
     filtre = filtre?.toLowerCase()
      ...
      if (filters?.includes(filtre?.toLowerCase())) {
        if (queue?.filters?.has(filtre)) {
          ...
          embed?.setDescription(`Efeito : **{filter}**, Status Aplicado: **{status}**`.replace("{filter}", filtre).replace("{status}", "❌"))
        ...
        embed?.setDescription(`❌ Não foi possível encontrar o filtro!!`.replace("{filters}", filters?.map(mr => `\`${mr}\``).join(", ")))
