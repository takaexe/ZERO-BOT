...
return interaction?.reply({ content: "Rate Limited.", ephemeral: true })
    ...
    function cmd_loader() {
        if (interaction?.type === InteractionType.ApplicationCommand) {
            ...
            return interaction?.reply({ content: '🔴 Taxa limitada'.replace("{channel_filter}", channel_filter), ephemeral: true }).catch(e => { })
            }
        }
            }
            ...
            if (!interaction?.member?.voice?.channelId) return interaction?.reply({ content: `🔴 Junte-se ao canal de voz primeiro!!`, ephemeral: true }).catch(e => { })
            ...
            return interaction?.reply({ content: `🔴 Deve estar no mesmo canal!!`, ephemeral: true }).catch(e => { })
            }
            }
            }
            ...
            return interaction?.reply({ content: `▶️ Permissões ausentes: **${props?.permissions?.replace("0x0000000000000020", "MANAGE GUILD")?.replace("0x0000000000000800", "SEND MESSAGES") || "SEND MESSAGES"}**`, ephemeral: true });
            }
            } catch (e) {
